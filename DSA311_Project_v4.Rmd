...
ggplot(heart_data, aes(y = age , x =oldpeak )) + 
  geom_point(aes(shape=num, col = num), size=5)
...
## limited by model specifications 

# glm.def1 <- glm(num~ sex+ cp + trestbps+ restecg + thalach+ exang + oldpeak+ slope+ ca+ thal, data=heart_data, family=binomial)
# summary(glm.def1)
# 
# glm.def2 <- glm(num~ sex+ cp + trestbps+  thalach+ exang + oldpeak+ slope+ ca+ thal, data=heart_data, family=binomial)
# summary(glm.def2)
# 
# glm.def3 <- glm(num~ sex+ cp + trestbps+ exang + oldpeak+ slope+ ca+ thal, data=heart_data, family=binomial)
# summary(glm.def3)
...
confusionMatrix(factor(prob<0.45), factor(heart.test$num=="One"), positive="TRUE")
...
library(pROC)
ROC = roc(pca.test$num=="One" ~ prob.pca, data = pca.test)
plot(ROC,print.thres=c(0.25,0.40,0.75),legacy.axes=F, print.auc = TRUE,col="Coral2")

confusionMatrix(factor(prob.pca<0.40), factor(pca.test$num=="One"), positive="TRUE")

################ Scaled data


scale.data <- read.csv("Simple_Encoded_Scaled.csv", stringsAsFactors = TRUE)
attach(scale.data)

train.scale <- sample(1:nrow(scale.data), nrow(scale.data)*0.8)

scale.train <- scale.data[train.scale,]
scale.y.train <- scale.data$num[train.scale]
scale.test <- scale.data[-train.scale,] 
scale.y.test <- scale.data$num[-train.scale]

set.seed(123)
head(scale.train)

glm.scale.def4 <- glm(num~ sex.1 + cp.2 +cp.3 +cp.4 + trestbps+ exang.1 + oldpeak+ slope+ ca+ thal, data=scale.train, family=binomial)
summary(glm.scale.def4)

prob.scale = predict(glm.scale.def4,type=c("response"),scale.test) # find predicted probabilities
scale.test$prob = prob.scale #add predicted probabilities to dataframe

library(pROC)
ROC = roc(scale.test$num=="One" ~ prob.scale, data = scale.test)
plot(ROC,print.thres=c(0.25,0.5,0.75),legacy.axes=F, print.auc = TRUE,col="Coral2")

confusionMatrix(factor(prob.scale<0.5), factor(scale.test$num=="One"), positive="TRUE")
...
library(pROC)
ROC = roc(encoded.raw.test$num=="One" ~ prob.raw, data = encoded.raw.test)
plot(ROC,print.thres=c(0.25,0.41,0.55,0.6,0.75),legacy.axes=F, print.auc = TRUE,col="Coral2")

confusionMatrix(factor(prob.raw<0.41), factor(encoded.raw.test$num=="One"), positive="TRUE")

confusionMatrix(factor(prob.raw<0.55), factor(encoded.raw.test$num=="One"), positive="TRUE")

confusionMatrix(factor(prob.raw<0.6), factor(encoded.raw.test$num=="One"), positive="TRUE")
